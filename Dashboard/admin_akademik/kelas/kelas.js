...
document.addEventListener("DOMContentLoaded", () => {
  ...
  initLogoutLogic(); // Inisialisasi logika logout lebih awal
...
function jalankanFilterDanCari() {
  ...
  filteredKelasData = allKelasData.filter((item) => {
    ...
    const currentTahunId =
      item.tahun_akademik_id ||
      (item.tahun_akademik ? item.tahun_akademik.id : "");
...
async function fillEditDropdowns() {
  ...
  const targetProdiElements = [
    "edit-prodi",
    "prodi_id",
    "id_prodi",
    "program_studi",
  ];
  const targetTahunElements = [
    "edit-tahun",
    "tahun_akademik_id",
    "id_tahun_akademik",
    "tahun_akademik",
  ...
  if (el.tagName === "SELECT") {
    ...
    for (let option of el.options) {
      if (
        ...
        option.value.toLowerCase().trim() === lowerValue
      ) {
  ...
  try {
    ...
    if (kapasitasInput) {
      kapasitasInput.value =
        item.kapasitas_mahasiswa ||
        item.kapasitas ||
        item.maksimal_mahasiswa ||
        "";
    }
    ...
    const prodiId =
      item.prodi_id ||
      (item.prodi ? item.prodi.id || item.prodi.id_prodi : "") ||
      item.id_prodi;
    const tahunId =
      item.tahun_akademik_id ||
      (item.tahun_akademik
        ? item.tahun_akademik.id || item.tahun_akademik.id_tahun_akademik
        : "") ||
...
async function handleUpdateKelas() {
  ...
  if (!id) {
    const elId =
      document.getElementById("edit-id-kelas") ||
  ...
  const isDuplicate = allKelasData.find((kelas) => {
    const kodeExisting = (kelas.kode_kelas || kelas.kode || "")
      .trim()
      .toLowerCase();
    ...
    return (
      kodeExisting === inputKodeKelas.toLowerCase() && currentId !== String(id)
    );
  });

  if (isDuplicate) {
    alert(
      `Peringatan: Kode Kelas "${inputKodeKelas}" sudah digunakan oleh kelas lain! Silakan gunakan kode yang berbeda.`,
    );
  ...
  let secureStatus =
    typeof rawStatus === "string" ? rawStatus.toLowerCase().trim() : "aktif";

  const payload = {
    kode_kelas: inputKodeKelas, // Gunakan variabel yang sudah di-trim di atas
    nama_kelas: getValue(["edit-nama-kelas", "nama_kelas"]),
    prodi_id:
      parseInt(getValue(["edit-prodi", "prodi_id", "id_prodi"])) ||
      getValue(["edit-prodi", "prodi_id", "id_prodi"]),
    tahun_akademik_id:
      parseInt(
        getValue(["edit-tahun", "tahun_akademik_id", "id_tahun_akademik"]),
      ) || getValue(["edit-tahun", "tahun_akademik_id", "id_tahun_akademik"]),
    kapasitas_mahasiswa:
      parseInt(getValue(["edit-kapasitas", "kapasitas_mahasiswa"])) ||
      getValue(["edit-kapasitas", "kapasitas_mahasiswa"]),
    status: secureStatus,
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (result.errors) {
        ...
        alert(
          `Gagal memperbarui data: ${result.message || "Status kode HTTP " + res.status}`,
        );
      }
    }
  } catch (error) {
...
// ==========================================
// 8. LOGIKA MODAL LOGOUT (KLUAR)
// ==========================================
function initLogoutLogic() {
  // Gunakan event delegation agar tombol logout tetap bisa diklik
  document.addEventListener("click", (e) => {
    const trigger =
      e.target.closest(".menu-item.logout") ||
      e.target.closest(".item-logout") ||
      e.target.closest("#logout-trigger") ||
      e.target.closest("a[href*='baru.html']");

    if (trigger) {
      e.preventDefault();
      // Mencari berdasarkan ID atau Class (sesuai akademik.css)
      const modalOverlay =
        document.getElementById("logout-modal") ||
        document.querySelector(".logout-modal-overlay");

      if (modalOverlay) {
        modalOverlay.classList.add("show");
      } else {
        console.warn(
          "⚠️ [Logout] Modal tidak ditemukan di DOM. Pastikan HTML modal ada di halaman ini.",
        );
      }
    }
  });

  // Ambil referensi modal untuk setup tombol internalnya
  const modalOverlay =
    document.getElementById("logout-modal") ||
    document.querySelector(".logout-modal-overlay");
  ...
  // Sembunyikan Pop-up jika tombol "Batal" diklik
  if (btnBatal) {
  ...
  // Sembunyikan Pop-up jika area luar kotak diklik
  ...
  // Eksekusi Hapus Sesi dan Redirect
  if (btnKonfirmasi) {
    btnKonfirmasi.addEventListener("click", (e) => {
      e.preventDefault();
      localStorage.clear();
      sessionStorage.clear();
      // Path disesuaikan: keluar dari folder kelas/ ke root loginbaru/
      window.location.href = "../../../loginbaru/baru.html";
    });
  }
}
